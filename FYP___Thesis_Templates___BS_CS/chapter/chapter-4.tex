...
\begin{tikzpicture}[
  font=\footnotesize,
  actor/.style={rectangle, draw, rounded corners, minimum width=1.6cm, minimum height=0.55cm},
  lifeline/.style={rectangle, draw, minimum width=0.15cm, minimum height=3.2cm},
  msg/.style={-{Stealth[length=1.8mm]}, thick}
]
\node[actor] (u) {User};
\node[actor, right=2.0cm of u] (sp) {SPA};
\node[actor, right=2.0cm of sp] (api) {API};
\node[actor, right=2.0cm of api] (db) {DB};
\draw[msg] ($(u.east)+(0,-0.35)$) -- node[above]{open UI} ($(sp.west)+(0,-0.35)$);
\draw[msg] ($(sp.east)+(0,-0.65)$) -- node[above]{GET /api/... + JWT} ($(api.west)+(0,-0.65)$);
\draw[msg] ($(api.east)+(0,-0.95)$) -- node[above]{SQL} ($(db.west)+(0,-0.95)$);
